- [Устройства](#устройства)
  ...
  - [Жизненный цикл устройства](#жизненный-цикл-устройства)
    - [Отключение сервиса](#отключение-сервиса)
    - [Автоперезапуск сервиса](#автоперезапуск-сервиса)
...
Если устройство дальше не может работать в этих условиях - оно может отключить сервис. 

```ts
this.terminate(new Error('Я так больше не могу'))
```

Если у сервиса настроена автоматическая перезагрузка - сервис будет перезапущен. 

Иногда необходимо сделать так, что бы после ошибки он больше не перезапускался. Например, когда требуется вмешательство человека для решения проблемы.

Тогда лучше обернуть вашу ошибку:

```js
// Перехват ошибки
catch (error){
  // Сообщаем что мы не хотим автоматический перезапуск
  const nError =  ErrorManager.make('CTR_IGNORE_SERVICE_AUTORELOAD').add(error)
  this.terminate(nError)
}
```

### Автоперезапуск сервиса

Если включен автоматический перезапуск сервиса в мета-данных, поведение перезапуска будет зависить от типа исключения. 

Если ошибка произошла на следующих этапах - сервис не будет перезапущен

 - Инициализация устройства
 - Создание портов
 - Создание соединений
 - При вызове `preProcess()` или `process()` или `processPromise()`
  
То есть до момента полной загрузки `Container`. 
