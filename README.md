...
- [Окружение для разработки](./docs/DevelopmentEnvironment.md)
- [Создание первого сервиса](./docs/FirstService.md)
- [Дополнительные примеры сервисов](https://github.com/VRack2/vrack2-example)
- [Музыкальный плеер с интерфейсом](./docs/PlayerService.md)
- [BME280 (i2c гигротермодавлёметр) с интерфейсом](./docs/BME280.md)
...
- [Системные события](./docs/SystemEvents.md)
...
## Последние изменения 1.2.0

 - Обновление документации - добавлен раздел `Системные события` (./docs/SystemEvents.md)
 - Добавлены вызовы системных событий (см. раздел `Системные события`)
 - Теперь в мета-данных сервиса можно указать список событий основного контейнера, на которые будет подписан сервис
 - Теперь внутри дочернего сервиса, можно подписаться на `Service.parent.event` для получения пробрасываемых внутрь событий
 - Обновление описания всех основных устройств 
 - В интерфейс метаданных сервиса добавлен необязательный параметр  **parentChannels?**: *Array* <string> для списка каналов
 - Удален лишний файл Interval - который использовался в тестировании на начальных этапах, но по какой то причине небыл удален
